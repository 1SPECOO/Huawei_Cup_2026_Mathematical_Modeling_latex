...
%填写摘要
\begin{abstract}
链路速率建模是无线通信系统中实现高效资源调度与自适应传输的关键环节，随着 MIMO-OFDM 技术的广泛应用，无线链路逐渐呈现出显著的频率选择性与空间选择性，这直接导致原始信道状态信息数据维度激增，难以直接为系统级调度提供有效支撑。经典的方案EESM指数加权的形式难以全面刻画不同调制编码方式下误块率与SINR的非线性关系，也无法适应CSI随时间与空间变化带来的统计漂移。本文通过建立参数拟合模型，提出新的速率估计方法，并基于LGBM，GCN，蒸馏模型，Tucker分解等理论方法，建模新的速率估计模型，实现了对信道传输速率的评估。

针对问题一，在未开启波束赋形的条件下，首先根据\textbf{最小二乘算法}与\textbf{逻辑线性回归理论}提出基于最小二乘的参数拟合与逻辑回归的EESM模型的速率估计;随后，根据\textbf{极大似然估计}和\textbf{有序判决回归理论}，提出极大似然的有序判决回归的EESM模型进行参数拟合和速率估计；然后根据\textbf{知识蒸馏理论}与\textbf{LightGBM模型理论}，提出基于LGBM的蒸馏式参数拟合方案。对比分析三种模型的ACC,MAE,F1，选择最优模型的拟合参数作为参考，并在此基础上对验证集各载波单元进行SINR计算与SINR等效映射。

针对问题二，在未开启波束赋形的条件下，首先根据\textbf{图神经网络模型}和\textbf{SVD分解理论}建立基于图神经网络的速率估计模型；随后根据数据集特征与\textbf{LightGBM模型理论}并在此基础上改进，提出基于物理先验表征的改进LGBM的速率估计模型。对比分析两个模型的ACC,loss,MAE，选择最优模型对验证集信道传输速率进行估并可视化结果。

针对问题三，在开启波束赋形的条件下，首先根据\textbf{SVD分解理论}与\textbf{Tucker分解理论}，实现对波束赋形的显式建模；其次根据\textbf{图理论}，\textbf{GraphTransformer模型}与\textbf{多头注意力机制}，提出基于显式TxBF与频域图建模的链路速率有序分类模型，并在此基础上，增加时延域与频域多尺度特征信息，提出基于时延域与多尺度频域特征增强的改进TxBF-GT模型；最后根据\textbf{智能反射表面理论}与\textbf{图卷积神经网络模型}构建基于智能反射面与图神经网络的自适应链路速率预测模型。对比分析三个模型的ACC,loss,MAE，选择最优模型对验证集信道传输速率进行估并可视化结果。
